package io.junq.examples.test.common.service;
...
import io.junq.examples.common.util.order.OrderByName;

public abstract class AbstractServiceIntegrationTest<T extends INameableEntity>
		extends AbstractRawServiceIntegrationTest<T> {
	// tests
	...
	/**/public final void whenResourcesAreRetrievedPaginatedAndSorted_thenResourcesAreIndeedOrdered() {
		...
		// When
		final List<T> resourcesPaginatedAndSorted = getApi().findAllPaginatedAndSorted(0, 4, SearchField.name.name(),
